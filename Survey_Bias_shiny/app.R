...
choicesA <- c("Select bias type:", "filtering", "leading", "anchoring")
choicesB <- c("Select bias type:", "confusing", "unbiased", "loaded")
choicesC <- c("Select bias type:", "filtering", "confusing", "unbiased", "loaded")
  ...
  dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        menuItem("Biases", tabName = "Biases"),
    ...
    dashboardBody(
      tabItems(
        ...
        tabItem(
          ...
          withMathJax(),
          h1("Instructions"),
          p("The goal of this app is to illustrate different types of biases
            that occur in the wording of survey questions."),
          br(),
          h2("Biases"),
            p("On the Biases page, click the + sign next to each question to read a definition 
              of the named bias."),
            p("Under each definition, you will find a problematic example. 
              Think carefully about how you would change the example survey question in order to remove the bias.
              You can write down your solution and discuss it with your peers."),
            p("Under each problematic example, you wil see a \"Remove the bias\" button. Click on the button to reveal a possible solution."),
            p("Learn the definitions of the various biases; you will need that knowledge for the quiz"),
          br(),
          h2("Quiz"),
            p("On the Quiz page, you will be asked to identify example questions with one of the biases covered on the Biases page.
              This is a three-level quiz, with each level consisting of 4 questions that contain a bias."),
            p("As the levels get harder, some questions will contain multiple biases; in these cases you should select the most
              prevelant bias."),
            p("There is a timer that will start as soon as you begin the quiz.
            ...
            p("In order to move from level to level, you will need to fully
        ...
        tabItem(
          tabName = "Biases",
          withMathJax(),
          h2("Common Survey Wording Biases"),
          # tabsetPanel(
          #   type = "tabs",
            ### Bias Examples tab
            # tabPanel(
            #   title = "Wording Bias",
              ...
              p("When designing a survey questionnaire, one needs to pay careful
                attention to the wording of the questions. It is easy to inadvertently
                bias a dataset through inappropriately formulated survey questions.
                Below are a few examples of biased question wording to watch out for.
                It is by no means a comprehensive list of common biases. "),
              br(),
              ### Row 1 ----
              fluidRow(
                column(
                  width = 6,
                  box(
                    title = HTML(paste0("<b>","Leading questions", "</b>")),
                    collapsible = TRUE,
                    collapsed = TRUE,
                    width = 12,
                    p("Leading question tend to push the respondent in the direction of a specific answer.
                      They are often used deliberately to gather support for a
                      specific cause or opinion. Consider the bias example below
                      then reveal the correct wording."),
                    br(),
                    p(HTML(paste0("<b>", "Problematic Example:", "</b>")), br(),
                      br(),
                      "It is hard for today's university graduates to have a bright
                ...
                column(
                  width = 6,
                  box(
                    title = HTML(paste0("<b>","Filtering", "</b>")),
                    collapsible = TRUE,
                    collapsed = TRUE,
                    width = 12,
                    ...
                    p(HTML(paste0("<b>", "Problematic Example:", "</b>")), br(),
                      br(),
                      "What is your opinion of the UK's current Prime Minister?",
                      tags$ol(
                        type = "a",
                        tags$li("Favorable"),
                        tags$li("Unfavorable")
              ...
              fluidRow(
                column(
                  width = 6,
                  box(
                    title = HTML(paste0("<b>","Anchoring", "</b>")),
                    collapsible = TRUE,
                    collapsed = TRUE,
                    width = 12,
                    ...
                    p(HTML(paste0("<b>", "Problematic Example:", "</b>")), br(),
                      br(),
                      "Knowing that the population of the UK is around 67.5 million,
                ...
                column(
                  width = 6,
                  box(
                    title = HTML(paste0("<b>","Loaded words", "</b>")),
                    collapsible = TRUE,
                    collapsed = TRUE,
                    width = 12,
                    p("The use of words such as 'forbid', 'control', 'ban',
                      'outlaw', or 'restraint' may introduce unintended bias. 
              ...
              fluidRow(
                column(
                  width = 6,
                  box(
                    title = HTML(paste0("<b>","Confusing Complexity: Double-barreled", "</b>")),
                    collapsible = TRUE,
                    collapsed = TRUE,
                    width = 12,
                    p("One way a question can be confusingly complex is if it
                    ...
                    p(HTML(paste0("<b>", "Problematic Example:", "</b>")), br(),
                      br(),
                      "Do you think that health care workers and the elderly
                ...
                column(
                  width = 6,
                  box(
                    title = HTML(paste0("<b>","Confusing Complexity: Double Negatives", "</b>")),
                    collapsible = TRUE,
                    collapsed = TRUE,
                    width = 12,
                    p("Another way in which questions can be confusingly complex
                    ...
                    p(HTML(paste0("<b>", "Problematic Example:", "</b>")), br(),
                      br(),
                      "Do you disagree that smoking in public places should not be banned?",
                    ),
                    br(), br(),
                    bsButton(
        ...
        tabItem(
          ...
          tabsetPanel(
            ...
            ## Start Tab ----
            tabPanel(
              title = "Survey Bias Quiz",
              h2("Survey Bias Quiz"),
              p("You will be asked to identify example questions with one of the biases covered on the Biases page.
              There are three levels, with each level consisting of 3 or 4 questions that contain a bias."),
              p("As the levels get harder, some questions will contain multiple biases; in these cases you should select the most
              prevelant bias."),
              p("There is a timer that will start as soon as you begin the quiz.
                For each question you get wrong, you will be deducted 2 points
            ...
            tabPanel(
              title = "Final Scores",
              h3("Final Scores"),
              p("Congratulations on finishing the quiz!"),
              br(),
...
# Biases Page - Remove bias ----
      ...
      output$doubleBarrelExample <- renderUI({
        p("Who should have priority in receiving the COVID-19 vaccination?",
          tags$ol(
            ...
            tags$li("The elderly"),
            tags$li("Both health care workers and the elderly"),
            tags$li("Neither")
          )
        )
      })
  ...
  observeEvent(input$fixDoubleNeg, {
    output$doubleNegExample <- renderText({
      "Do you think smoking in public places should be banned?"
    })
  })
  ...
  observeEvent(input$go2, {
    tempBankA <- bank %>%
      filter(Type %in% c("filtering", "leading", "anchoring")) %>%
      slice_sample(n = 3)

    tempBankB <- bank %>%
      filter(Type %in% c("confusing", "unbiased", "loaded")) %>%
      slice_sample(n = 3)

    tempBankC <- bank %>%
      filter(Type %in% c("filtering", "confusing", "unbiased", "loaded")) %>%
